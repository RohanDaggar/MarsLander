...
def runThreeD():
    ...
    # Verlet integration
    # 2 less itterations needed as the initial condition is t=0, and t=dt is done above
    for t in t_array[:-2]:

        # calculate new position and velocity
        # next_position is equal to position[:,[-1]] at this point and for the next line too
        Force = - G * M * m * next_position / np.linalg.norm(next_position)**3
        # next_position is equal to position[:,[-1]] at this point and for the next line too
        next_position = 2*next_position - \
            position[:, [-2]] + dt * dt * Force / m
